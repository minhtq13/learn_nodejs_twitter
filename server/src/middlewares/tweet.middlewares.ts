...
export const createTweetValidator = validate(
  checkSchema({
      ...
      custom: {
        options: (value, { req }) => {
          ...
          if (type === TweetType.Tweet && value !== null) {
            throw new Error(TWEET_MESSAGES.PAREN_ID_MUST_BE_NULL);
          }
          return true;
        },
      },
    ...
    content: {
      ...
      custom: {
        options: (value, { req }) => {
          ...
          // Nếu type là comment, quotetweet, tweet và không có mentions và hastags thì content phải là string và không được rỗng
          if (
    ...
    mentions: {
      ...
      custom: {
        options: (value, { req }) => {
          // Yêu cầu mỗi phần tử trong array là string là user_id
...
// Nếu k dùng try catch thì phải dùng wrapRequestHandler
export const audienceValidator = wrapRequestHandler(async (req: Request, res: Response, next: NextFunction) => {
  const tweet = req.tweet as Tweet;
  if (tweet.audience === TweetAudience.TwitterCicrle) {
    ...
    // Kiểm tra tài khoản tác giả có ổn (bị khoá hay bị xoá chưa) không?
    ...
    // Kiểm tra người xem tweet này có trong Twiteer Cicrle của tác giả hay không?
